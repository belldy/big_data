...
numbers = journalists.select {|x| x =~ /[0123456789]/}
...
aude = journalists.select { |x| x.include?('aude') || x.include?('Aude')}
#puts aude
aude_count = aude.length
puts aude_count

# Combien commencent par une majuscule (première lettre juste après le @) ?

first_maj = journalists.select {|x| x =~ /^@[[:upper:]]/}
#puts first_maj
first_maj_count = first_maj.length
puts first_maj_count

#Combien contiennent une majuscule ?

have_maj = journalists.select { |x| x =~ /[[:upper:]]/}
have_maj_count = have_maj.length
puts have_maj_count

# Combien y a-t-il de underscore _ dans tous les pseudos confondus ?

underscore = journalists.select {|x| x =~ /[_]/}
#puts underscore
underscore_count = underscore.length
puts underscore_count

# Trie la liste de handle par ordre alphabétique.

sorted = journalists.sort

# Quels sont les 50 handles les plus courts de ce array ?

smallest = journalists.min(50) {|a, b| a.length <=> b.length }
puts smallest
